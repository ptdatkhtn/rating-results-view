...
const App = ({
        ...
        const result = modifiedNodes.map(node => {
            ...
            for (let i = 0; i < modifiedNodes.length; ++i) {
                if (modifiedNodes[i].id !== node.id) {
                    ...
                    if (rangeX <= rangeNearby && rangeY <= rangeNearby) {
                        const ranX = rangeNearby < rangeX * 2 ? -0.2 : 0.2
                        const ranY = rangeNearby < rangeY * 2 ? -0.2 : 0.2
    ...
    const nodeListAsMedian = React.useMemo(() => {
        ...
        return isRelative ? modifyRelativeNodes(nodes, listPoint()) : modifyValueNodes(nodes)
    ...
    useEffect(() => {
        ...
        const myCircleAvg = scatterSvg.append('g').append('clipPath')
            .selectAll('circle')
            .data(nodeListAsAverage)
            .join('circle')
            .attr('stroke', d => {
        ...
        const fpIconMedian = scatterSvg.append('g')
            ...
            .attr('height', fpIconSize)
            .attr('width', fpIconSize)
        ...
        const zoomX = d3.zoom().scaleExtent([1, 30]).translateExtent([[0, 0], [containerWidth, containerHeight]])
        const zoomY = d3.zoom().scaleExtent([1, 30]).translateExtent([[0, 0], [containerWidth, containerHeight]])
        ...
        // active zooming
        const zoom = d3.zoom().scaleExtent([1, 30]).translateExtent([[0, 0], [containerWidth, containerHeight]]).on("zoom", function (e) {
            try {
        ...
        <div style={{ width: '100%' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: '56px', paddingRight: '60px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginRight: '-12px' }}>
                    ...
                    <button disabled={decreaseLevel === 1} onClick={handleIncreaseNodes}>Increase nodes</button>
                    <button disabled={decreaseLevel <= 0.6} onClick={handleDecreaseNodes}>Decrease nodes</button>
                </div>
            </div>
